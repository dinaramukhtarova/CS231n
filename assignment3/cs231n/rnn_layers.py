...
def rnn_backward(dh, cache):
  ...
  for t in reversed(xrange(1, T)):
    dx[:, t, :], dprev_h, dWx_cur, dWh_cur, db_cur = rnn_step_backward(dh[:, t, :] + dprev_h, (x[:, t, :], h[:, t-1, :], Wx, Wh, b, h[:, t, :]))
    dWx += dWx_cur
    dWh += dWh_cur
    db += db_cur
  dx[:, 0, :], dh0, dWx_cur, dWh_cur, db_cur = rnn_step_backward(dh[:, 0, :] + dprev_h, (x[:, 0, :], h0, Wx, Wh, b, h[:, 0, :]))
  dWx += dWx_cur
  dWh += dWh_cur
  db += db_cur
  ##############################################################################
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(xrange(1, T)):
    dx[:, t, :], dprev_h, dprev_c, dWx_cur, dWh_cur, db_cur = lstm_step_backward(dh[:, t] + dprev_h, dc[:, t] + dprev_c, cache[t])
    dWx += dWx_cur
    dWh += dWh_cur
    db += db_cur
  dx[:, 0, :], dh0, dc0, dWx_cur, dWh_cur, db_cur = lstm_step_backward(dh[:, 0] + dprev_h, dc[:, 0] + dprev_c, cache[0])
  dWx += dWx_cur
  dWh += dWh_cur
  db += db_cur
  ##############################################################################
