...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    ...
    for j in xrange(num_classes):
        if j == y[i]:
            dW[:, j] += (X[i] * (np.exp(scores[j]) - np.sum(np.exp(scores)))) / np.sum(np.exp(scores))
        else:
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  indicator[np.arange(num_train), y] = np.sum(np.exp(scores), axis = 1)
  dW += X.T.dot((np.exp(scores) - indicator) / np.reshape(np.sum(np.exp(scores), axis = 1), (num_train, 1)))
  loss /= num_train
  dW /= num_train
  loss += 0.5 * reg * np.sum(W * W)
  dW += reg * W
